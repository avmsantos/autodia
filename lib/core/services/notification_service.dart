...
class NotificationService extends GetxService {
  ...
  static const _prefKeyAtivas = 'notificacoes_ativas';

  /// Preferência do usuário — pode desativar tudo sem precisar mexer em
  /// cada lembrete individualmente. Persistida entre aberturas do app.
  final RxBool notificacoesAtivas = true.obs;

  late final SharedPreferences _prefs;

  Future<NotificationService> init() async {
    _prefs = await SharedPreferences.getInstance();
    notificacoesAtivas.value = _prefs.getBool(_prefKeyAtivas) ?? true;
  ...
  /// Liga/desliga notificações globalmente. Ao desativar, cancela tudo que
  /// já estava agendado. Ao reativar, reagenda os lembretes de todos os
  /// veículos a partir do zero.
  Future<void> alternarNotificacoes(bool ativo, AppDatabase db) async {
    notificacoesAtivas.value = ativo;
    await _prefs.setBool(_prefKeyAtivas, ativo);
    ...
    for (final veiculo in veiculos) {
      await rescheduleAllForVehicle(db, veiculo);
    }
  }
  ...
  /// Se `dueDate` já passou ou é null, não agenda nada. Se o usuário
  /// desativou notificações globalmente, também não agenda.
  Future<void> scheduleReminderNotification({
    ...
    if (!notificacoesAtivas.value) return;
  ...
  /// Cancela TODAS as notificações agendadas — usado na exclusão de conta
  /// e ao desativar notificações globalmente.
  Future<void> cancelAll() => _plugin.cancelAll();

  /// Dispara uma notificação imediata, sem agendamento — útil pro próprio
  /// usuário confirmar que as notificações estão funcionando no aparelho.
